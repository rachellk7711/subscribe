...
// 네이버 캘린더용 웹 등록 링크 생성
export const getNaverCalendarLink = (sub: Subscription) => {
  const today = new Date();
  const year = today.getFullYear();
  const billingMonth = sub.billing_month ? sub.billing_month - 1 : today.getMonth();
  const eventDate = new Date(year, billingMonth, sub.billing_date - 1, 10, 0, 0);
  ...
  // 네이버 형식: YYYYMMDDHHmm
  const formatNaver = (date: Date) => {
    const pad = (n: number) => n.toString().padStart(2, '0');
    return `${date.getFullYear()}${pad(date.getMonth() + 1)}${pad(date.getDate())}${pad(date.getHours())}${pad(date.getMinutes())}`;
  };

  const start = formatNaver(eventDate);
  const end = formatNaver(new Date(eventDate.getTime() + 30 * 60 * 1000));
  
  const title = encodeURIComponent(`[결제알림] ${sub.service_name}`);
  const context = encodeURIComponent([
    `💰 금액: ${sub.amount.toLocaleString()}${sub.currency === 'USD' ? '$' : '원'}`,
    `📝 메모: ${sub.memo || '없음'}`
  ].join(' / '));

  return `https://calendar.naver.com/event/main.nhn?method=insert&title=${title}&startDateTime=${start}&endDateTime=${end}&content=${context}&isAllDay=false`;
};
