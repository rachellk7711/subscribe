// Recovery Build: Fixed Sidebar + Normal Button Width + Red & White
...
function App() {
  ...
  return (
    <div className="flex h-screen bg-[#f7f7f7] text-[#222222] font-sans overflow-hidden">
      {/* PC 사이드바: 고정 너비와 배경색을 명시적으로 부여 */}
      <aside className="hidden lg:flex flex-col w-72 bg-white border-r border-gray-200 shrink-0 h-full">
        <div className="p-8">
          <h1 className="text-2xl font-black text-[#ff385c] tracking-tighter flex items-center gap-2">
            <LayoutDashboard size={28} /> 관리자
          </h1>
          <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">우리 집 통합 고정비 관리</p>
        </div>
        <nav className="flex-1 px-4 space-y-1 overflow-y-auto">
          {['전체', ...CATEGORIES].map(cat => (
            <button key={cat} onClick={() => setActiveTab(cat)} className={cn("w-full flex items-center justify-between px-5 py-3.5 rounded-xl text-sm font-bold transition-all text-left", activeTab === cat ? "bg-[#ff385c] text-white shadow-md" : "text-gray-500 hover:bg-gray-100")}>
        ...
        <div className="p-6 border-t border-gray-100 bg-white">
          <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-2xl border border-gray-100">
            ...
            <div><p className="text-xs font-black">MASTER</p><p className="text-[10px] text-gray-400">Household Pro</p></div>
          </div>
        </div>
      </aside>
        ...
        <div className="fixed inset-0 z-[100] lg:hidden">
          ...
          <div className="absolute inset-y-0 left-0 w-72 bg-white flex flex-col shadow-2xl">
            ...
            <div className="flex-1 px-4 py-6 space-y-1">
              {['전체', ...CATEGORIES].map(cat => (
                <button key={cat} onClick={() => { setActiveTab(cat); setIsMobileMenuOpen(false); }} className={cn("w-full flex items-center gap-3 px-5 py-4 rounded-xl font-bold", activeTab === cat ? "bg-[#ff385c] text-white" : "text-gray-500 hover:bg-gray-100")}>
      ...
      {/* 메인 컨텐츠 영역 */}
      <main className="flex-1 flex flex-col min-w-0 bg-[#f7f7f7] overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 lg:p-10 space-y-10 pb-32">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
            <div>
              ...
              <p className="text-gray-500 font-medium mt-1">총 {filteredSubs.length}개의 항목이 관리되고 있습니다.</p>
            </div>
            {/* 버튼의 너비가 쭉 늘어나지 않도록 고정 또는 최대 너비 지정 */}
            <button onClick={() => { setEditingSub(null); setIsModalOpen(true); }} className="w-full sm:w-auto min-w-[180px] bg-[#ff385c] text-white px-8 py-4 rounded-2xl font-black text-lg shadow-lg hover:bg-[#e31c5f] transition-all active:scale-95">
              + 지출 항목 추가
            </button>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 bg-white border border-gray-200 rounded-[32px] p-8 lg:p-12 shadow-sm relative overflow-hidden">
              ...
              <div className="mt-10 flex flex-wrap gap-3">
                <div className="px-4 py-2 rounded-full bg-gray-50 border border-gray-200 text-xs font-bold text-gray-500">환율: ₩{Math.round(exchangeRate)}</div>
                <div className="px-4 py-2 rounded-full bg-[#ff385c]/10 border border-[#ff385c]/20 text-xs font-black text-[#ff385c]">납부 진행률: {Math.round((subscriptions.filter(s => s.is_paid).length / (subscriptions.length || 1)) * 100)}%</div>
              </div>
            </div>
              ...
              <table className="w-full text-left border-collapse min-w-[800px]">
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredSubs.map((sub) => {
                    ...
                    return (
                      <tr key={sub.id} className={cn("group transition-colors", isManualUnpaid ? "bg-red-50" : "hover:bg-gray-50")}>
                        ...
                        <td className="px-8 py-7">
                          <div className="flex flex-col">
                            <span className="font-black text-lg text-gray-900 flex items-center gap-2">
                              {sub.service_name}
                              {sub.is_variable && <span className="text-[9px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded font-black uppercase">변동비</span>}
                            </span>
                        ...
                        <td className="px-8 py-7 font-black text-gray-900">
                        ...
                        <td className="px-8 py-7 text-right">
                          <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all">
                            <button onClick={() => setCalendarMenuId(calendarMenuId === sub.id ? null : sub.id)} className="p-2.5 bg-white border border-gray-200 rounded-xl hover:bg-[#ff385c] hover:text-white transition-all"><Calendar size={18} /></button>
                          ...
                          {calendarMenuId === sub.id && (
                            <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-2xl shadow-2xl z-[100] overflow-hidden text-left shadow-airbnb">
                              <a href={getNaverCalendarLink(sub)} target="_blank" className="block px-5 py-3.5 text-xs font-black text-green-600 hover:bg-green-50 border-b border-gray-100">네이버 캘린더 등록</a>
                              <a href={getGoogleCalendarLink(sub)} target="_blank" className="block px-5 py-3.5 text-xs font-black text-blue-600 hover:bg-blue-50 border-b border-gray-100">구글 캘린더 등록</a>
                              <button onClick={() => downloadICS(sub)} className="w-full text-left px-5 py-3.5 text-xs font-black text-gray-700 hover:bg-gray-50">ICS 파일 다운로드</button>
                            </div>
                          )}
                        </td>
                      </tr>
        ...
        {/* 모달 창 */}
        {isModalOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-md p-0 sm:p-6">
            <div className="bg-white rounded-t-[40px] sm:rounded-[40px] shadow-2xl w-full max-w-xl flex flex-col h-[90vh] sm:h-auto mt-auto sm:mt-0 overflow-hidden">
              <div className="px-10 py-8 border-b border-gray-100 flex justify-between items-center shrink-0">
                <h3 className="text-2xl font-black tracking-tight text-gray-900">{editingSub ? '지출 정보 수정' : '새 지출 항목 추가'}</h3>
                <button onClick={() => { setIsModalOpen(false); setEditingSub(null); }} className="p-3 bg-gray-50 rounded-2xl hover:bg-gray-100 transition-all"><X size={24} /></button>
              </div>
              <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-10 space-y-8 pb-32 sm:pb-10 bg-white">
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <label className="flex items-center gap-2 mt-3 cursor-pointer">
                      ...
                      <span className="text-xs font-bold text-gray-500">매달 금액이 달라짐 (변동비)</span>
                    </label>
                  </div>
                  <div>
                    <label className="block text-[11px] font-black text-gray-400 uppercase tracking-widest mb-2.5">통화</label>
                    <select name="currency" defaultValue={editingSub?.currency || 'KRW'} className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-5 py-4 font-black outline-none bg-white"><option value="KRW">KRW (₩)</option><option value="USD">USD ($)</option></select>
                  </div>
                  <div className="col-span-2 border-t border-gray-50 pt-4">
                    <label className="block text-[11px] font-black text-gray-400 uppercase tracking-widest mb-4">납부 방식 선택</label>
                    <div className="grid grid-cols-2 gap-4">
                      <label className={cn("flex flex-col p-4 rounded-2xl border-2 transition-all cursor-pointer", (editingSub?.payment_type === 'auto' || !editingSub) ? "border-[#ff385c] bg-[#ff385c]/5" : "border-gray-100")}>
                      ...
                      <label className={cn("flex flex-col p-4 rounded-2xl border-2 transition-all cursor-pointer", editingSub?.payment_type === 'manual' ? "border-red-500 bg-red-50" : "border-gray-100")}>
                        ...
                        <span className="text-[10px] text-gray-400 mt-1 font-bold">이체/지로 직접 결제</span>
                      </label>
                    </div>
                  </div>
                  <div className="col-span-2 grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select name="billing_cycle" defaultValue={editingSub?.billing_cycle || 'monthly'} onChange={(e) => setModalBillingCycle(e.target.value as 'monthly'|'yearly')} className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-5 py-3.5 font-black outline-none bg-white"><option value="monthly">매월</option><option value="yearly">매년</option></select>
                    </div>
                    {modalBillingCycle === 'yearly' && (
                      <div>
                        ...
                        <select name="annual_type" defaultValue={editingSub?.annual_type || 'split'} className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-5 py-3.5 font-black outline-none bg-white"><option value="split">12개월 분할</option><option value="single">일시불 (당월)</option></select>
                      </div>
                    )}
                  </div>
                  <div className="col-span-2 grid grid-cols-2 gap-4">
                    {modalBillingCycle === 'yearly' && (<div><label className="block text-xs font-black text-gray-400 mb-2">결제 월</label><select name="billing_month" defaultValue={editingSub?.billing_month || 1} className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-5 py-3.5 font-black bg-white">{Array.from({length: 12}, (_, i) => i + 1).map(m => <option key={m} value={m}>{m}월</option>)}</select></div>)}
                    <div className={modalBillingCycle === 'monthly' ? 'col-span-2' : ''}><label className="block text-xs font-black text-gray-400 mb-2">결제 일</label><select name="billing_date" defaultValue={editingSub?.billing_date || 1} className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-5 py-3.5 font-black bg-white">{Array.from({length: 31}, (_, i) => i + 1).map(d => <option key={d} value={d}>{d}일</option>)}</select></div>
                  </div>
                  <div className="col-span-2 grid grid-cols-2 gap-4 pb-20 sm:pb-0">
                    <div>
                      <label className="block text-[11px] font-black text-gray-400 uppercase tracking-widest mb-2.5">카테고리</label>
                      <select name="category" defaultValue={editingSub?.category || '디지털 구독'} className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-5 py-3.5 font-black bg-white">{CATEGORIES.map(cat => <option key={cat} value={cat}>{cat}</option>)}</select>
                    </div>
                    <div>
                      <label className="block text-[11px] font-black text-gray-400 uppercase tracking-widest mb-2.5">결제 수단</label>
                      <input name="payment_method" defaultValue={editingSub?.payment_method} type="text" placeholder="예: 신한카드 1234" className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-5 py-3.5 font-black bg-white" />
                    </div>
                  </div>
                </div>
                <div className="fixed sm:static bottom-0 left-0 right-0 p-8 sm:p-0 bg-white sm:bg-transparent border-t border-gray-100 sm:border-none flex flex-col sm:flex-row gap-4 mt-8">
                  ...
                  <button type="submit" disabled={isSubmitting} className="order-1 sm:order-2 flex-[2] bg-[#ff385c] text-white py-5 rounded-2xl font-black text-xl hover:bg-[#e31c5f] shadow-lg flex items-center justify-center gap-3">
